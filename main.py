...
def add_train(train_no, train_name, depature_date, start, end):
    c.execute("INSERT INTO trains (train_no, train_name, depature_date, start, end) VALUES (?, ?, ?, ?, ?)", 
              (train_no, train_name, depature_date, start, end))
    conn.commit()
    create_seat_table(train_no)

def delete_train(train_no, depature_date):
    train_query = c.execute("SELECT * FROM trains WHERE train_no = ? AND depature_date = ?", (train_no, depature_date))
    train_data = train_query.fetchone()
    if train_data:
        c.execute("DELETE FROM trains WHERE train_no = ? AND depature_date = ?", (train_no, depature_date))
        conn.commit()
...
def create_seat_table(train_no):
    ...
    for i in range(1, 51):
        ...
        c.execute(f"INSERT INTO seats_{train_no} (seat_number, seat_type, booked, passenger_name, passenger_age, passenger_gender)"
                  "VALUES (?, ?, ?, '', '', '')", (i, val, 0))
    conn.commit()
...
def allocate_next_available_seat(train_no, seat_type):
    seat_query = c.execute("SELECT seat_number FROM seats_{} WHERE booked = 0 AND seat_type = ? ORDER BY seat_number".format(train_no), (seat_type,))
    res = seat_query.fetchall()
    if res:
        return res[0][0]

def book_ticket(train_no, name, age, gender, seat_type):
    train_query = c.execute("SELECT * FROM trains WHERE train_no = ?", (train_no,))
    train_data = train_query.fetchone()
    if train_data:
        seat_number = allocate_next_available_seat(train_no, seat_type)
        if seat_number:
            c.execute(f"""UPDATE seats_{train_no} 
                        ...
                        passenger_age = ?,
                        passenger_gender = ?
                        WHERE seat_number = ?""", (seat_type, name, age, gender, seat_number))
            conn.commit()
...
def train_dest(start, end):
    train_query = c.execute("SELECT * FROM trains WHERE start = ? AND end = ?", (start, end))
    ...
    if functions == "ADD TRAIN":
            ...
            if train_number:
                ...
                if train_data:
                    ...
                    st.table(pd.DataFrame([train_data], columns=[
                        "Train Number", "Train Name", "Depature Date", "Starting Destination", "Ending Destination"]))
                else:
        ...
        if st.button("Search by Destinations"):
            if starting_destination and ending_destination:
                train_data = train_dest(starting_destination, ending_destination)
                if train_data:
